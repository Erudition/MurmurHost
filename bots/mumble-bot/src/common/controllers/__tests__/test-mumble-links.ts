...
import { linkMumbleUser } from "../../../test-utils/mumble-link";
import { createUserWithToken } from "../../../test-utils/token";
import { createUser } from "../../../test-utils/user";
import { MumbleLink } from "../../models/mumble-link";
...
describe("mumble-links controller", () => {
    ...
    describe("DELETE /mumble-link/:id", () => {
        ...
        it("responds 404 with an unknown link", async () => {
            const response = await api()
                .delete("/mumble-link/ad9fa629-082b-4dcb-b34c-2150bcd15376")
                .set("authorization", `Bearer ${token.id}`);
        ...
        it("responds 403 when trying to delete another users's link", async () => {
            const response = await api()
                .delete(`/mumble-link/${mumbleLink.id}`)
        ...
        it("deletes the specified link", async () => {
            ...
            const getResponse = await api().get("/mumble-links").set("authorization", `Bearer ${token.id}`);
    ...
    describe("GET /mumble-links", () => {
        ...
        it("fetches a list of all mumble links", async () => {
            const { user: otherUser1, token: otherUser1Token } = await createUserWithToken({
                name: "user1",
                email: "user1@example.com",
            } as User);
            const { user: otherUser2, token: otherUser2Token } = await createUserWithToken({
                name: "user2",
                email: "user2@example.com",
            } as User);
            ...
            const { token } = await createUserWithToken();
            const response = await api().get("/mumble-links").set("authorization", `Bearer ${token.id}`);
            expect(response.body).toMatchObject({
                data: [
                    {
                        mumbleId: 4,
