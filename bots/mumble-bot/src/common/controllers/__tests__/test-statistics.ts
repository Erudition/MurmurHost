...
import { createSound, rateSound } from "../../../test-utils/sound";
import { createTag, tagSound } from "../../../test-utils/tag";
import { createUserWithTokenManually } from "../../../test-utils/token";
import { createUserManually } from "../../../test-utils/user";
    ...
    [
        ...
        describe(`GET ${url}`, () => {
            ...
            it("responds 200 and the expected result with an empty database", async () => {
                const response = await api().get(url).set("authorization", `Bearer ${token.id}`);
