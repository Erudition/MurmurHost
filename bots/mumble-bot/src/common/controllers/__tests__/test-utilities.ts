...
describe("utilities controller", () => {
    ...
    describe("GET /channel-tree", () => {
        ...
        it("fetches a tree of the server's channels", async () => {
            ...
            const response = await api().get("/channel-tree").set("authorization", `Bearer ${token.id}`);
    ...
    describe("GET /mumble-users", () => {
        ...
        it("fetches a list of connected mumble users", async () => {
            const { token } = await createUserWithToken();
            const response = await api().get("/mumble-users").set("authorization", `Bearer ${token.id}`);
    ...
    describe("POST /shut-up", () => {
        ...
        it("clears the queue", async () => {
            const response = await api().post("/shut-up").set("authorization", `Bearer ${token.id}`);
            expect(response.status).toBe(200);
