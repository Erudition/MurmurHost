...
describe("tags controller", () => {
    ...
    describe("GET /tags", () => {
        ...
        it("fetches a list of all tags", async () => {
            ...
            const response = await api().get(`/tags`).set("authorization", `Bearer ${token.id}`);
    ...
    describe("POST /tags", () => {
        it("responds 401 without a valid token", async () => {
            const response = await api().post(`/tags`).send({ name: "First Tag" });
            expect(response.body).toEqual({ message: "Unauthorized." });
            expect(response.status).toBe(401);
        });
        ...
        it("creates a new tag", async () => {
            ...
            const listResponse = await api().get(`/tags`).set("authorization", `Bearer ${token.id}`);
