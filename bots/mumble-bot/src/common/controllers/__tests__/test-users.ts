...
describe("users controller", () => {
    ...
    describe("GET /users", () => {
        ...
        it("returns a list of users when logged in", async () => {
            ...
            const response = await api().get("/users").set("authorization", `Bearer ${token.id}`);
    ...
    describe("GET /user/:id", () => {
        ...
        it("responds 404 for unknown user", async () => {
            ...
            const response = await api()
                .get(`/user/65888dad-7250-4756-bd8d-ed3375007405`)
        ...
        it("fetches a user with a specific id", async () => {
            const { user, token } = await createUserWithToken();
            const response = await api().get(`/user/${user.id}`).set("authorization", `Bearer ${token.id}`);
            expect(response.body).toEqual({
                data: {
                    ...pick(["name", "id"], user),
                    avatarUrl:
    ...
    describe("post /user/:id", () => {
        ...
        it("responds 401 without a valid token", async () => {
            const response = await api().post(`/user/${adminUser.id}`).send({ name: "New name" });
            expect(response.body).toEqual({ message: "Unauthorized." });
            expect(response.status).toBe(401);
        });

        it("responds 401 for unknown user without a valid token", async () => {
            const response = await api().post(`/user/65888dad-7250-4756-bd8d-ed3375007405`).send({ name: "New name" });
            expect(response.body).toEqual({ message: "Unauthorized." });
            expect(response.status).toBe(401);
        });
        ...
        it("responds 404 for unknown user", async () => {
            const response = await api()
                .post(`/user/65888dad-7250-4756-bd8d-ed3375007405`)
        ...
        describe("as an admin", () => {
            ...
            it("can revoke admin status from other users", async () => {
                await tsdi.get(Database).getRepository(User).update(otherUser.id, { admin: true });
                const response = await api()
                    .post(`/user/${otherUser.id}`)
                    .set("authorization", `Bearer ${adminToken.id}`)
                    .send({ admin: false });
