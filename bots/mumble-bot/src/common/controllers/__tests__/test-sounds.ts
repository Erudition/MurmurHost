...
import { createSound, createSoundWithCreatorAndSpeaker, getSound, rateSound } from "../../../test-utils/sound";
...
import { createTag, tagSound } from "../../../test-utils/tag";
...
describe("sounds controller", () => {
    ...
    describe("GET /sound/:id", () => {
        ...
        it("responds 404 for unknown sound", async () => {
            ...
            const response = await api()
                .get(`/sound/65888dad-7250-4756-bd8d-ed3375007405`)
        ...
        it("fetches a sound with a specific id", async () => {
            ...
            const response = await api().get(`/sound/${sound.id}`).set("authorization", `Bearer ${token.id}`);
    ...
    describe("POST /sound/:id/tags", () => {
        ...
        it("responds 404 for unknown sound", async () => {
            const response = await api()
                .post(`/sound/65888dad-7250-4756-bd8d-ed3375007405/tags`)
                .set("authorization", `Bearer ${token.id}`)
                .send({ id: tag.id });
            expect(response.body).toEqual({ message: `No sound with id "65888dad-7250-4756-bd8d-ed3375007405"` });
            expect(response.status).toBe(404);
        ...
        it("responds 409 if already tagged with a tag", async () => {
            await tagSound(sound, tag, token);
            await api().post(`/sound/${sound.id}/tags`).set("authorization", `Bearer ${token.id}`).send({ id: tag.id });
            const response = await api()
                .post(`/sound/${sound.id}/tags`)
                .set("authorization", `Bearer ${token.id}`)
                .send({ id: tag.id });
    ...
    describe("DELETE /sound/:id/tag/:tagId", () => {
        ...
        it("responds 404 for unknown sound", async () => {
            const response = await api()
                .delete(`/sound/65888dad-7250-4756-bd8d-ed3375007405/tag/${tag.id}`)
                .set("authorization", `Bearer ${token.id}`);
            expect(response.body).toEqual({ message: `No sound with id "65888dad-7250-4756-bd8d-ed3375007405"` });
            expect(response.status).toBe(404);
        });
        ...
        it("responds 404 for unknown tag", async () => {
            const response = await api()
                .delete(`/sound/${sound.id}/tag/65888dad-7250-4756-bd8d-ed3375007405`)
                .set("authorization", `Bearer ${token.id}`);
            expect(response.body).toEqual({ message: `No tag with id "65888dad-7250-4756-bd8d-ed3375007405"` });
            expect(response.status).toBe(404);
        });
        ...
        it("removes the tag from the specified sound", async () => {
            const response = await api()
                .delete(`/sound/${sound.id}/tag/${tag.id}`)
                .set("authorization", `Bearer ${token.id}`);
    ...
    describe("POST /sound/:id", () => {
        ...
        it("responds 404 for unknown sound", async () => {
            const response = await api()
                .post(`/sound/65888dad-7250-4756-bd8d-ed3375007405`)
                .set("authorization", `Bearer ${token.id}`);
            expect(response.body).toEqual({ message: `No sound with id "65888dad-7250-4756-bd8d-ed3375007405"` });
            expect(response.status).toBe(404);
        });
    ...
    describe("DELETE /sound/:id", () => {
        ...
        it("responds 404 for unknown sound", async () => {
            const response = await api()
                .delete(`/sound/65888dad-7250-4756-bd8d-ed3375007405`)
                .set("authorization", `Bearer ${token.id}`);
            expect(response.body).toEqual({ message: `No sound with id "65888dad-7250-4756-bd8d-ed3375007405"` });
            expect(response.status).toBe(404);
        });
        ...
        it("responds 400 if deleting a deleted sound", async () => {
            await api().delete(`/sound/${sound.id}`).set("authorization", `Bearer ${token.id}`);
            const response = await api().delete(`/sound/${sound.id}`).set("authorization", `Bearer ${token.id}`);
    ...
    describe("GET /sounds", () => {
        ...
        beforeEach(async () => {
            ...
            tags = [await createTag("Tag A", userA.token), await createTag("Tag B", userA.token)];
            sounds = [
                await createSound({
                    ...
                    used: 100,
                    duration: 3.5,
                    created: new Date("2020-10-10T20:00:00Z"),
                } as Sound),
                await createSound({
                    ...
                    used: 15,
                    duration: 3.2,
                    created: new Date("2020-10-10T21:00:00Z"),
                } as Sound),
                await createSound({
                    ...
                    source: "upload",
                    duration: 0.5,
                    created: new Date("2020-10-10T22:00:00Z"),
                } as Sound),
            ];
        ...
        it("returns all sounds without any parameters", async () => {
            const response = await api().get("/sounds").set("authorization", `Bearer ${userA.token.id}`);
            expect(response.body).toMatchObject({
                data: {
                    totalSounds: 3,
        ...
        it("returns all sounds matching a creator", async () => {
            const response = await api()
                .get(`/sounds?creator=${userA.user.id}`)
        ...
        describe("returns all sounds with specified tags", () => {
            ...
            it("matches both tags", async () => {
                const response = await api()
                    .get(`/sounds?tags=${tags[1].id},${tags[0].id}`)
                    .set("authorization", `Bearer ${userA.token.id}`);
                expect(response.body.data).toMatchObject({
                    totalSounds: 1,
        ...
        it("returns all sounds matching a start date", async () => {
            const response = await api()
                .get(`/sounds?startDate=${sounds[1].created.toISOString()}`)
                .set("authorization", `Bearer ${userA.token.id}`);
            expect(response.body.data).toMatchObject({
                totalSounds: 2,
                sounds: [responseSounds[1], responseSounds[2]],
        ...
        it("returns all sounds matching an end date", async () => {
            const response = await api()
                .get(`/sounds?endDate=${sounds[1].created.toISOString()}`)
                .set("authorization", `Bearer ${userA.token.id}`);
            expect(response.body.data).toMatchObject({
                totalSounds: 2,
                sounds: [responseSounds[0], responseSounds[1]],
        ...
        it("returns all sounds matching a search string", async () => {
            const response = await api().get("/sounds?search=one").set("authorization", `Bearer ${userA.token.id}`);
            expect(response.body.data).toMatchObject({
                totalSounds: 2,
                sounds: [responseSounds[1], responseSounds[2]],
            });
        ...
        it("returns all sounds matching a source", async () => {
            const response = await api().get("/sounds?source=upload").set("authorization", `Bearer ${userA.token.id}`);
            expect(response.body.data).toMatchObject({
                totalSounds: 1,
        ...
        it("sorts the sounds by updated", async () => {
            const response = await api().get("/sounds?sort=updated").set("authorization", `Bearer ${userA.token.id}`);
            expect(response.body.data).toMatchObject({
                totalSounds: 3,
        ...
        it("sorts the sounds by created", async () => {
            const response = await api().get("/sounds?sort=created").set("authorization", `Bearer ${userA.token.id}`);
            expect(response.body.data).toMatchObject({
                totalSounds: 3,
                sounds: [responseSounds[0], responseSounds[1], responseSounds[2]],
            });
        ...
        it("sorts the sounds by duration", async () => {
            const response = await api().get("/sounds?sort=duration").set("authorization", `Bearer ${userA.token.id}`);
            expect(response.body.data).toMatchObject({
                totalSounds: 3,
        ...
        it("sorts the sounds by usage count ascending", async () => {
            const response = await api()
                .get("/sounds?sort=used&sortDirection=asc")
                .set("authorization", `Bearer ${userA.token.id}`);
            expect(response.body.data).toMatchObject({
                totalSounds: 3,
                sounds: [responseSounds[1], responseSounds[2], responseSounds[0]],
        ...
        it("sorts the sounds by usage count descending", async () => {
            const response = await api()
                .get("/sounds?sort=used&sortDirection=desc")
                .set("authorization", `Bearer ${userA.token.id}`);
            expect(response.body.data).toMatchObject({
                totalSounds: 3,
        ...
        it("sorts the sounds by rating descending", async () => {
            const response = await api()
                .get("/sounds?sort=rating&sortDirection=desc")
                .set("authorization", `Bearer ${userA.token.id}`);
            expect(response.body.data).toMatchObject({
                totalSounds: 3,
                sounds: [responseSounds[0], responseSounds[2], responseSounds[1]],
        ...
        it("sorts the sounds by rating ascending", async () => {
            const response = await api()
                .get("/sounds?sort=rating&sortDirection=asc")
                .set("authorization", `Bearer ${userA.token.id}`);
            expect(response.body.data).toMatchObject({
                totalSounds: 3,
                sounds: [responseSounds[1], responseSounds[2], responseSounds[0]],
        ...
        it("with limit and offset", async () => {
            const response = await api()
                .get("/sounds?sort=used&sortDirection=desc&limit=1&offset=1")
                .set("authorization", `Bearer ${userA.token.id}`);
            expect(response.body.data).toMatchObject({
                totalSounds: 3,
                limit: 1,
    ...
    describe("POST /sounds/youtube", () => {
        ...
        it("returns 400 with an invalid youtube url", async () => {
            const response = await api().post("/sounds/youtube").set("authorization", `Bearer ${token.id}`).send({
    ...
    describe("POST /sounds/upload", () => {
        ...
        it("returns 400 with invalid base64", async () => {
            const response = await api().post("/sounds/upload").set("authorization", `Bearer ${token.id}`).send({
                filename: "some-file.mp3",
    ...
    describe("POST /sound/:id/fork", () => {
        ...
        it("returns 404 with an unknown sound", async () => {
            const response = await api()
                .post(`/sound/953fac26-f2a2-484d-9743-79779be7ec14/fork`)
                .set("authorization", `Bearer ${token.id}`)
                .send({
                    description: "New description",
                    actions: [{ action: "crop", start: 1, end: 2 }],
                    overwrite: true,
                });
        ...
        [true, false].forEach((overwrite) => {
            it(`creates a new sound and ${overwrite ? "deletes" : "doesn't delete"} the old sound`, async () => {
                ...
                const oldSoundResponse = await api()
                    .get(`/sound/${sound.id}`)
                    .set("authorization", `Bearer ${token.id}`);
