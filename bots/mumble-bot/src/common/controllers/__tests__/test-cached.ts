...
describe("cached controller", () => {
    ...
    describe("GET /cached", () => {
        ...
        it("fetches a list of all cached audios", async () => {
            const response = await api().get(`/cached`).set("authorization", `Bearer ${token.id}`);
    ...
    describe("DELETE /cached/:id", () => {
        ...
        it("responds 404 with a valid token with unkown id", async () => {
            const response = await api()
                .delete(`/cached/89c11873-8691-4196-9d18-be9df458b5f2`)
        ...
        it("deletes the cached audio", async () => {
            const responseDelete = await api()
                .delete(`/cached/${cachedAudio1.id}`)
                .set("authorization", `Bearer ${token.id}`);
            expect(responseDelete.status).toBe(200);
            const responseGet = await api().get(`/cached`).set("authorization", `Bearer ${token.id}`);
