...
import { createSound } from "../../../test-utils/sound";
...
describe("playlists controller", () => {
    ...
    let playlist1Response: unknown;
    let playlist2Response: unknown;
    ...
    describe("GET /playlists", () => {
        ...
        it("filter by creator", async () => {
            const response = await api()
                .get(`/playlists?creator=${otherUser.id}`)
        ...
        it("filter by name", async () => {
            const response = await api().get(`/playlists?search=another`).set("authorization", `Bearer ${token.id}`);
            expect(response.status).toBe(200);
            expect(response.body).toMatchObject({
                data: {
                    totalPlaylists: 1,
        ...
        it(`fetches a list of all playlists with limit and offset`, async () => {
            const response = await api()
                .get("/playlists?sort=created&sortDirection=asc&offset=1&limit=1")
                .set("authorization", `Bearer ${token.id}`);
            expect(response.status).toBe(200);
            expect(response.body).toMatchObject({
                data: {
        ...
        [
            ...
            it(`fetches a list of all playlists sorted by "${sort}"`, async () => {
                const response = await api()
                    .get(`/playlists?sort=${sort}&sortDirection=asc`)
                    .set("authorization", `Bearer ${token.id}`);
                expect(response.status).toBe(200);
                expect(response.body).toMatchObject({
                    data: {
    ...
    describe("GET /playlist/:id", () => {
        ...
        it("responds 404 with an unknown playlist", async () => {
            const response = await api()
                .get("/playlist/626256d1-820b-4c6d-a68d-59bbc447dcc3")
                .set("authorization", `Bearer ${token.id}`);
        ...
        it("fetches the specified playlist", async () => {
            const response = await api().get(`/playlist/${playlist1.id}`).set("authorization", `Bearer ${token.id}`);
            expect(response.status).toBe(200);
            expect(response.body).toMatchObject({
    ...
    describe("POST /playlists", () => {
        it("responds 401 without a valid token", async () => {
            const response = await api().post(`/playlists`).send(playlist1);
            expect(response.body).toEqual({ message: "Unauthorized." });
            expect(response.status).toBe(401);
        });
        ...
        it("creates a new playlist", async () => {
            ...
            const listResponse = await api()
                .get(`/playlists?sort=created&sortDirection=desc`)
                .set("authorization", `Bearer ${token.id}`);
    ...
    describe("POST /playlist/:id", () => {
        it("responds 401 without a valid token", async () => {
            const response = await api().post(`/playlist/${playlist1.id}`).send({});
            expect(response.body).toEqual({ message: "Unauthorized." });
            expect(response.status).toBe(401);
        });

        it("responds 401 with an unkown playlist with an invalid token", async () => {
            const response = await api().post("/playlist/d33512a2-c0c5-4736-a908-fd90ca40af1d").send({});
            expect(response.body).toEqual({ message: "Unauthorized." });
            expect(response.status).toBe(401);
        });
        ...
        it("responds 404 with an unkown playlist", async () => {
            const response = await api()
                .post("/playlist/d33512a2-c0c5-4736-a908-fd90ca40af1d")
                .set("authorization", `Bearer ${token.id}`)
                .send({});
        ...
        it("updates the playlist", async () => {
            const response = await api()
                .post(`/playlist/${playlist1.id}`)
                .set("authorization", `Bearer ${token.id}`)
                .send({ description: "Some new description" });
            expect(response.status).toBe(200);
            const getResponse = await api().get(`/playlist/${playlist1.id}`).set("authorization", `Bearer ${token.id}`);
