...
import { visualizeAudioFile } from "../visualizer/visualizer";

setupWinston();

@command({ description: "Start the worker." })
export default class VisualizerCommand extends Command {
    // tslint:disable-line
    private getVisualizationFilename = (file): string => `${file}.png`;
    private killed = false;

    private async generateVisualization(file: string): Promise<void> {
    ...
    private shouldGenerateVisualization(path: string): boolean {
        return (
            !path.endsWith("png") &&
            !path.endsWith("json") &&
    ...
    private async generateMissingVisualizations(path: string): Promise<void> {
        ...
        for (const [index, file] of missingFiles.entries()) {
            if (this.killed) {
                return;
            }
            const percent = ((100 * index) / missingFiles.length).toFixed(2);
            info(
                `Visualizing ${file} to ${this.getVisualizationFilename(file)}. ` +
    ...
    @metadata
    public async execute(config: VisualizerConfig): Promise<void> {
        if (!config.load()) {
            return;
        }
