...
export class AudioAnalyzer {
     ...
     */
    public analyze(chunk: Float32Array): void {
     ...
     */
    public draw(canvas): void {
        if (this.list.length < 2) {
            return;
        }
        const overallMax = this.list.reduce(
            (result, value) => (result > value.amplitude ? result : value.amplitude),
            0,
        );
        ...
        for (let i = 1; i < values.length; i++) {
            ...
            const color = Color.hsl(360 * (0.05 + relFreq * 0.95), 100, 50).toString();
